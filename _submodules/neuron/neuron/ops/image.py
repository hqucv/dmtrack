...
def show_image_with_label(img, bboxes=None, bbox_fmt='ltrb', colors=None,
               thickness=3, fig=1, delay=1, max_size=640,
               visualize=True, cvt_code=cv2.COLOR_RGB2BGR):
    if cvt_code is not None:
        img = cv2.cvtColor(img, cvt_code)
    ...
    if bboxes is not None:
        ...
        for i, bbox in enumerate(bboxes):
            ...
            if len(bbox) == 5:
                pt1 = (int(bbox[0]), int(bbox[1]))
                pt2 = (int(bbox[2]), int(bbox[3]))
                img = cv2.rectangle(img, pt1, pt2, color.tolist(), thickness=tl, lineType=cv2.LINE_AA)
                # label
                img = cv2.putText(img, '%.2f' % bbox[4], (pt1[0], pt1[1]+5), 0, tl / 3, [255, 255, 0], thickness=tf, lineType=cv2.LINE_AA)
            else:
                pts = bbox.reshape(-1, 2)
                img = cv2.polylines(img, [pts], True, color.tolist(), thickness)
