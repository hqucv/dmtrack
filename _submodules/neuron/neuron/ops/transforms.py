...
def to_gray_scale(img, color_weights):
    img_gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    return np.stack([img_gray, img_gray, img_gray], axis=2)


def blur(img, filter, filter_size, probability):
    if random.random() < probability:
        if isinstance(img, torch.Tensor):
            sz = img.shape[2:]
            im1 = F.conv2d(img.view(-1, 1, sz[0], sz[1]), filter[0], padding=(filter_size[0], 0))
            return F.conv2d(im1, filter[1], padding=(0, filter_size[1])).view(-1, sz[0], sz[1])
        else:
