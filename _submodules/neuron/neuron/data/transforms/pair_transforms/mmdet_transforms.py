...
class PhotometricDistort(_PairTransform):
    ...
    def _process_query(self, img, meta, bboxes=None):
        img = ops.photometric_distort(img, swap_channels=self.swap_channels)
...
class GrayScale(_PairTransform):
    ...
    def _process_query(self, img, meta, bboxes=None):
        if random.random() < self.probability:
            img = ops.to_gray_scale(img, color_weights=self.color_weights)
            self.to_gray = True
        else:
            self.to_gray = False
        return img, meta, bboxes
...
class Blur(_PairTransform):

    def __init__(self, sigma, blur_probability):
    ...
    def _process_gallary(self, img, meta, bboxes=None):
        img = ops.blur(img, self.filter, self.filter_size, self.blur_probability)
        return img, meta, bboxes
...
class ExtraPairTransforms(Compose):
    ...
    def __init__(self,
        ...
        transforms = []
        if with_grayscale:
            transforms += [GrayScale(gray_probability)]
        if with_photometric:
        ...
        if with_blur:
            transforms += [Blur(sigma, blur_probability)]
