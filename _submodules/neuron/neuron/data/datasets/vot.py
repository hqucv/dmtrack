...
class VOT(SeqDataset):
    ...
    def _construct_seq_dict(self, root_dir, list_file):
        ...
        if os.path.exists(osp.join(root_dir, seq_names[0], 'annotations', 'groundtruth.txt')):
            anno_files = [osp.join(root_dir, s, 'annotations', 'groundtruth.txt')
                          for s in seq_names]
        else:
            anno_files = [osp.join(root_dir, s, 'annotation','groundtruth.txt')
                               for s in seq_names]
