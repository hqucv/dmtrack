...
class GOT10kEval(Evaluator):
    ...
    def run(self, tracker, visualize=False, return_all=False):
        ...
        for s, (img_files, target) in enumerate(self.dataset):
            ...
            for r in range(self.repetitions):
                ...
                bboxes, times = tracker.forward_test(
                    img_files, target['anno'][0], visualize=visualize, return_all=return_all)
    ...
    def report(self, tracker_names, plot_curves=False, return_all=False):
        ...
        if self.dataset.subset == 'test':
            ...
            for name in tracker_names:
                ...
                for s, (_, target) in enumerate(self.dataset):
                    ...
                    if return_all:
                        for bbox in bboxes:
                            bboxes, max_indexes = self.convert_to_best_bboxes(bbox, anno)
                            tmp_bboxes.append(bboxes)
                            tmp_max_indexes.append(max_indexes)
                        bboxes = tmp_bboxes
                        seq_max_indexes_l.append(tmp_max_indexes)
                ...
                if return_all:
                    all_indexes = []
                    for seq_max_indexes in seq_max_indexes_l:
                        all_indexes.append(seq_max_indexes[0])
                    seq_max_indexes = np.concatenate(all_indexes)
                    print('average best sample num:' + str(np.mean(seq_max_indexes)))
                    print('max best sample num:' + str(np.max(seq_max_indexes)))
                    print('min best sample num:' + str(np.min(seq_max_indexes)))

                    unique_data = np.unique(np.reshape(seq_max_indexes, (-1)))

                    resdata = []
                    for ii in unique_data:
                        resdata.append(np.reshape(seq_max_indexes, (-1)).tolist().count(ii))
