...
class OTB_Eval(Evaluator):
    ...
    def run(self, tracker, visualize=None, return_all=False):
        ...
        for s, (img_files, target) in enumerate(self.dataset):
            ...
            bboxes, times = tracker.forward_test(
                img_files, init_bbox, visualize=visualize, return_all=return_all)
    ...
    def report(self, tracker_names, plot_curves=None, return_all=False):
        ...
        for name in tracker_names:
            ...
            for s, (_, target) in enumerate(self.dataset):
                ...
                if return_all:
                    bboxes, max_indexes = self.convert_to_best_bboxes(bboxes, anno)
                    seq_max_indexes.append(max_indexes)
            ...
            if return_all:
                seq_max_indexes = np.concatenate(seq_max_indexes, axis=0)
