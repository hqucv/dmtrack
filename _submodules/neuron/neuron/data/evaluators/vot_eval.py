...
__all__ = ['VOT_Eval', 'EvaluatorVOT', 'VOTLT_Eval', 'EvaluatorVOTLT']
...
class VOT_Eval(Evaluator):
    ...
    def run_supervised(self, tracker, visualize=False, return_all=False):
            ...
            for r in range(self.repetitions):
                ...
                for f, img_file in enumerate(img_files):
                    ...
                    if f == 0:
                        ...
                        bbox = tracker.update(frame, return_all=return_all)
        ...
        for s, (img_files, target) in enumerate(self.dataset):
            ...
            for r in range(self.repetitions):
                ...
                record_file = os.path.join(
                    ...
                    '%s_%03d.txt' % (seq_name, r + 1))
                record__confidence_file = os.path.join(
                    self.result_dir, tracker.name, seq_name,
                    '%s_%03d_confidence.value' % (seq_name, r + 1))
                if os.path.exists(record_file):
                    ops.sys_print('  Found results, skipping %s' % seq_name)
                    continue
                ...
                for f, img_file in enumerate(img_files):
                    ...
                    if visualize:
                        if len(bboxes[-1]) == 5:
                            ops.show_image(img, bboxes[-1][:-1])
                        else:
                            ops.show_image(img)

                # record results
                self._record(record_file, bboxes, times)
                self._record_confidence(record__confidence_file, bboxes)

    def report(self, tracker_names, plot_curves=False):
        raise NotImplementedError(
            'VOT Long-term eval in offical tookit.')
    ...
    def _record(self, record_file, bboxes, times):
        ...
        for bbox in bboxes:
            if len(bbox) == 1:
                ...
                lines.append(str.join(',', ['%.4f' % t for t in bbox[:-1]]))
...
@registry.register_module
class EvaluatorVOTLT(VOTLT_Eval):

    def __init__(self, root_dir=None, version=2018, **kwargs):
        dataset = VOT(root_dir, version, anno_type='default')
        super(EvaluatorVOTLT, self).__init__(dataset, **kwargs)
